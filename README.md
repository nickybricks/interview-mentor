...
| **Chat** | Conversation session — linked to a project, typed as `kickoff`, `gap_analysis`, `preparation`, or `mock_interview`, with status tracking (`active`/`completed`) |
...
| `PATCH` | `/api/chats/[id]` | Update chat status (`active`/`completed`) |
...
- [x] Kickoff completion overlay with delayed display (waits for stream end + 3s)
