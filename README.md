...
| Model | Purpose |
|---|---|
| **Project** | One job application — stores name, company, position, CV text, job description, gap analysis result, overall score |
| **Chat** | Conversation session — linked to a project, typed as `gap_analysis`, `preparation`, or `mock_interview` |
...
| **VectorDocument** | RAG embeddings — pgvector 1536-dim chunks for knowledge base and user documents |

Cascade deletion: Project → Chats → Messages. Project → Documents.

## RAG Pipeline

1. **Ingestion** — Documents are chunked (500 chars, 50 overlap), embedded with `text-embedding-3-small`, stored in `VectorDocument` via pgvector
2. **Query translation** — For each user message, 3 alternative query rephrasings are generated via LLM
3. **Retrieval** — All 4 queries (original + alternatives) search pgvector in parallel, results are deduplicated (highest similarity kept), top-k returned
...
3 tools defined in `lib/tools.ts` using `tool()` from `@langchain/core/tools` with Zod schemas:

| Tool | Purpose |
|---|---|
| `scoreAnswer` | Dedicated LLM call (temperature 0.2) evaluating answers across 5 dimensions: Substance, Structure, Relevance, Credibility, Differentiation (each 1–5, mapped to 1–10) |
| `getWeakAreas` | Queries flagged messages grouped by category, returns average scores and sample questions per weak area |
| `searchKnowledgeBase` | Searches RAG knowledge base via `retrieveContext()`, returns top 5 results with source, similarity, and preview |
