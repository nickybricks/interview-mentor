...
Scoring calibration framework for maintaining consistent and accurate interview answer scoring.

---

## Section 1: Calibration Metadata Schema

Track your calibration status (uncalibrated → calibrating → calibrated → miscalibrated), scoring drift observations, and cross-dimension root causes.
...
Run when 3+ real interview outcomes exist.
...
**Detection**: When the same root cause appears across multiple answers or sessions, it graduates from "per-answer observation" to "active root cause."

Example: "Conflict avoidance" detected affecting Substance and Differentiation across multiple answers → create entry:

| Root Cause | Affected Dimensions | Status | Treatment |
|---|---|---|---|
| Conflict avoidance | Substance, Differentiation | Active | Tension-mining drill on top 3 stories |
...
Apply these freshness rules when referencing intelligence data:
...
Role-specific drills use native scoring axes that don't map 1:1 to the core 5 dimensions. This mapping ensures role-drill performance feeds into trend analysis and calibration.
...
1. Score using the native drill axes
...
When outcomes show "advanced" or "offer":
...
- Update storybank notes for stories used in successful interviews
...
- Record confirmed success patterns for future reference
