...
Detailed scoring rubrics for evaluating interview answers across 5 dimensions.
...
When the same root cause appears across multiple answers, escalate it from a per-answer observation to a persistent coaching focus. The key principle: prescribe ONE intervention targeting the root cause itself, not separate drills for each affected dimension.
